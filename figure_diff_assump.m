...
NUM_ROW  = 5; % the first row for num of act users
drawPointMtx1 = zeros(NUM_ROW,1);%For Data correct ratio
...
% paramaters that are known in BS (for activity detection and data recovery)
par.nd =nd ;
par.mRS = mRS;
par.nRS = nRS;
par.kRS = kRS;
...
drawPointMtxROW = 1; 
for NUM_PILOT = [64 200]   % No. of pilot [64 200 300 400 500] 
    ...
    for NUM_ACT_USERS = 5:5:100
        ...
        for itr = 1:NUM_ITR
            ...
            % acUsr_pilot=[acUsrIndx;pilotIndx]; 
            % acUsr_sortpilot=sortrows(acUsr_pilot',2)'; 
            ...
            if NUM_PILOT == 64
                ...
                drawPointMtx1(2,drawPointMtxColumn) = drawPointMtx1(2,drawPointMtxColumn) + length(D_est)/ NUM_ACT_USERS;
            else  
                [B_est, H_est1] = OMP_blind(Y_pilot_rx,cwplt);            
                [A_est,D_est ] = Data_LS(B_est,H_est1,Y_data_rx, cwsbl,NUM_PILOT,acUsrIndx,acData,par);
                drawPointMtx1(3,drawPointMtxColumn) = drawPointMtx1(3,drawPointMtxColumn) + length(D_est)/ NUM_ACT_USERS;           
                H_est2= zeros(size(H_pilot));
                H_est2(pilot_choose,:)= cwplt(:,pilot_choose) \ Y_pilot_rx; 
                [A_est,D_est ] = Data_LS(pilot_choose,H_est2,Y_data_rx, cwsbl,NUM_PILOT,acUsrIndx,acData,par);
                drawPointMtx1(4,drawPointMtxColumn) = drawPointMtx1(4,drawPointMtxColumn) + length(D_est)/ NUM_ACT_USERS;           
                [A_est,D_est ] = Data_LS(pilot_choose,H_pilot,Y_data_rx, cwsbl,NUM_PILOT,acUsrIndx,acData,par);
                drawPointMtx1(5,drawPointMtxColumn) = drawPointMtx1(5,drawPointMtxColumn) + length(D_est)/ NUM_ACT_USERS;           
            end           
...
%save('figure_difass','drawPointMtx1')
