function [A_est,D_est] = Data_LS(B, H_est,Y_data_rx, cwsbl,NUM_PILOT,acUsrIndx,acData,par)
...
HX_pilot=zeros(NUM_PILOT,nd*nOfTfcb);
D = zeros(NUM_PILOT,nd*nOfTfcb);
eat_Data = zeros(NUM_ALL_USERS,nEvryUsrDataBits);

HX_pilot(B,:)= cwsbl(:,B) \ Y_data_rx;

H2 = mat2cell(H_est,[size(H_est,1)],NUM_RX*ones(1,nOfTfcb));
H2 = repelem(H2,1,nd);
H2 = cell2mat(H2);
